<script>
  ...
  onMount(async () => {
    ...
    const initial = {
      angle: 0,
      radius: Math.sqrt(
        Math.pow(initialCameraPosition.x, 2) +
          Math.pow(initialCameraPosition.z, 2)
      ),
      ...
      if (autoRotate && modelGroup) {
        if (lerpProgress < 1) {
          ...
          if (lerpProgress >= 1) {
            ...
            camera.position.x =
              lerpStart.x + (lerpEnd.x - lerpStart.x) * lerpProgress;
            camera.position.y =
              lerpStart.y + (lerpEnd.y - lerpStart.y) * lerpProgress;
            camera.position.z =
              lerpStart.z + (lerpEnd.z - lerpStart.z) * lerpProgress;
          }
