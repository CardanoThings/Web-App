export const load = async () => {
	return {
		...
		qrLibraryInfo: `
			...
			// 3. Search for "QRCode" by ricardocosme
			// 4. Install the library
			//
			// Library provides functions:
			// - qrcode_initText() - Initialize QR code with text data
			// - qrcode_getModule() - Get individual module (pixel) state
			// - qrcode_getBufferSize() - Calculate required buffer size
			//
			// Error Correction Levels:
			// - ECC_LOW - ~7% error correction
			// - ECC_MEDIUM - ~15% error correction (recommended)
			// - ECC_QUARTILE - ~25% error correction
			// - ECC_HIGH - ~30% error correction
			//
			// QR Code Versions:
			// - Version 1-40 (higher = more data capacity)
			// - Version 3 can store ~50-100 characters
			// - For CIP-13 URIs, version 3-4 is usually sufficient`
	};
};
